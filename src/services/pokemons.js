...
export const getPokemonsByTypes = async (pokemonType) => {
...
export const getPokemonByUrl = async (pokemonUrl) => {
    ...
    const pokemon = {
        id: data.id, 
        name: data.name, 
        types: formatingTypes(data.types),
        stats: formatingStats(data.stats),
...
const formatingTypes = (types) => {
    return types.map((type) => type.type.name)
};
const formatingStats = (stats) => {
